...
`ADMIN` 管理用户、角色、模板及所有数据；`MAINTAINER` 维护模板；`LEADER` 查看、修改、审核所有填报；`REPORTER` 只能按模板手工填报或导入并查看自己的数据。兼容保留 `EDITOR`、`VIEWER` 角色。登录后将返回的 token 作为 `Authorization: Bearer <token>` 发送。
...
- `GET/POST/PUT/DELETE /api/reports`：填报记录查询、创建、修改、删除；`PATCH /api/reports/{id}/review` 由管理员或领导审核。
...
- `GET /api/change-requests?status=PENDING`：填报人员仅查询本人申请，管理员和领导可查询全部；`PATCH /api/change-requests/{id}/approve|reject|cancel` 分别处理审批、驳回和申请人撤销。审批会复核 `baseUpdatedAt`，冲突时返回 `409`；驳回必须填写意见。
