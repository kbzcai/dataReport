...
public class ReportController {
    ...
    @PostMapping @ResponseStatus(HttpStatus.CREATED) @PreAuthorize("hasAnyRole('ADMIN','LEADER','REPORTER')") public ReportDtos.Response create(@Valid @RequestBody ReportDtos.Request request) { return reports.create(request); }
    ...
    @PostMapping("/batch") @ResponseStatus(HttpStatus.CREATED) @PreAuthorize("hasAnyRole('ADMIN','LEADER','REPORTER')") public List<ReportDtos.Response> createBatch(@Valid @RequestBody List<ReportDtos.Request> requests) { return reports.createBatch(requests); }
    @PutMapping("/{id}") @PreAuthorize("hasAnyRole('ADMIN','LEADER','REPORTER')") public ReportDtos.Response update(@PathVariable Long id, @Valid @RequestBody ReportDtos.Request request) { return reports.update(id, request); }
    ...
    @DeleteMapping("/{id}") @PreAuthorize("hasAnyRole('ADMIN','LEADER','REPORTER')") public void delete(@PathVariable Long id) { reports.delete(id); }
    ...
    @PostMapping("/import-confirm") @PreAuthorize("hasAnyRole('ADMIN','LEADER','REPORTER')") public ExcelDtos.ImportResult importConfirm(@RequestParam("file") MultipartFile file, @RequestParam String mapping) { return excel.confirmImport(file, mapping); }
    @PostMapping("/import") @PreAuthorize("hasAnyRole('ADMIN','LEADER','REPORTER')") public ExcelDtos.ImportResult importExcel(@RequestParam(required = false) Long templateId, @RequestParam(required = false) Long taskId, @RequestParam("file") MultipartFile file) { return templateId == null ? excel.importWorkbook(file) : excel.importRecords(templateId, taskId, file); }
