...
public class TemplateController {
    ...
    @GetMapping("/{id}") public TemplateDtos.Response get(@PathVariable Long id) { return templates.get(id); }
    @GetMapping("/{id}/versions") public List<TemplateDtos.VersionResponse> versions(@PathVariable Long id) { return templates.versions(id); }
    ...
    @PutMapping("/{id}") @PreAuthorize("hasAnyRole('ADMIN','MAINTAINER')") public TemplateDtos.Response update(@PathVariable Long id, @Valid @RequestBody TemplateDtos.Request request) { return templates.update(id, request); }
    @DeleteMapping("/{id}") @PreAuthorize("hasAnyRole('ADMIN','MAINTAINER')") public void delete(@PathVariable Long id) { templates.delete(id); }
    ...
    @GetMapping("/{id}/download") @PreAuthorize("hasAnyRole('ADMIN','MAINTAINER','LEADER','REPORTER','EDITOR','VIEWER')") public ResponseEntity<byte[]> downloadAlias(@PathVariable Long id) { return download(id); }
    @GetMapping("/{id}/excel-template") @PreAuthorize("hasAnyRole('ADMIN','MAINTAINER','LEADER','EDITOR','VIEWER')")
