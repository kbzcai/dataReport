...
public class ReportService {
    ...
    public ReportDtos.Response create(ReportDtos.Request request) {
        ...
        ReportRecord entity = new ReportRecord(); entity.setTemplate(template); entity.setTemplateVersion(version); entity.setTask(task); entity.setReporter(reporter); entity.setStatus(request.status() == null ? ReportStatus.DRAFT : request.status()); entity.setDataJson(writeData(data));
    ...
    public ReportDtos.Response update(Long id, ReportDtos.Request request) {
        ...
        String before = entity.getDataJson(); ReportTemplate template = templates.find(request.templateId()); ReportTask task = request.taskId() == null ? entity.getTask() : tasks.find(request.taskId()); validateTask(task, template, user); ReportTemplateVersion version = resolveVersion(template, task, request.templateVersionId() == null && entity.getTemplateVersion() != null ? entity.getTemplateVersion().getId() : request.templateVersionId()); Map<String, Object> data = applyDefaults(version, request.data()); validateData(version, data); validateUniqueValues(template, version, data, entity.getId()); entity.setTemplate(template); entity.setTemplateVersion(version); entity.setTask(task); entity.setDataJson(writeData(data)); if (request.status() != null) entity.setStatus(request.status()); ReportRecord saved = records.save(entity); syncValues(saved, version, data); audit.record(saved, "UPDATE", before, saved.getDataJson(), null); return toResponse(saved);
    }
