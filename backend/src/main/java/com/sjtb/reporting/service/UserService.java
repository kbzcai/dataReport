...
public class UserService {
    ...
    public UserDtos.Response create(UserDtos.CreateRequest request) {
        ...
        User user = new User(); user.setUsername(request.username()); user.setPassword(encoder.encode(request.password())); user.setRoles(request.roles()); return toResponse(users.save(user));
    }
    public UserDtos.Response update(Long id, UserDtos.UpdateRequest request) {
        ...
        if (request.roles() != null && !request.roles().isEmpty()) user.setRoles(request.roles()); if (request.enabled() != null) user.setEnabled(request.enabled()); return toResponse(user);
    }
    ...
    private UserDtos.Response toResponse(User user) { return new UserDtos.Response(user.getId(), user.getUsername(), user.getRoles(), user.isEnabled()); }
}
