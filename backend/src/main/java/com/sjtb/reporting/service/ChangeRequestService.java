...
public class ChangeRequestService {
    private final ReportChangeRequestRepository requests; private final ReportRecordRepository records; private final ReportService reports; private final TemplateService templates; private final ReportAuditService audit; private final CurrentUserService current; private final ObjectMapper mapper;
    public ChangeRequestService(ReportChangeRequestRepository requests, ReportRecordRepository records, ReportService reports, TemplateService templates, ReportAuditService audit, CurrentUserService current, ObjectMapper mapper) { this.requests = requests; this.records = records; this.reports = reports; this.templates = templates; this.audit = audit; this.current = current; this.mapper = mapper; }
    public ChangeRequestDtos.Response create(Long reportId, ChangeRequestDtos.CreateRequest input) {
        User user = current.current(); ReportRecord report = lockedReport(reportId);
    ...
    @Transactional(readOnly = true) public List<ChangeRequestDtos.Response> list(ChangeRequestStatus status) {
        User user = current.current(); boolean manager = manager(user);
        List<ReportChangeRequest> source = manager ? (status == null ? requests.findAll() : requests.findByStatusOrderByCreatedAtAsc(status)) : (status == null ? requests.findByRequesterIdOrderByCreatedAtDesc(user.getId()) : requests.findByRequesterIdAndStatusOrderByCreatedAtDesc(user.getId(), status));
    ...
    private ChangeRequestDtos.Response review(Long id, ChangeRequestDtos.ReviewRequest input, ChangeRequestStatus target) {
        User reviewer = current.current(); if (!manager(reviewer)) throw new ApiException(HttpStatus.FORBIDDEN, "Only a leader may review requests");
        ReportChangeRequest entity = find(id); if (entity.getStatus() != ChangeRequestStatus.PENDING) throw new ApiException(HttpStatus.BAD_REQUEST, "Only pending requests can be reviewed");
    ...
    private boolean manager(User user) { return user.getRoles().contains(Role.ADMIN) || user.getRoles().contains(Role.LEADER); }
